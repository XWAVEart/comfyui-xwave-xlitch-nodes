...
## 1. Node Self-Containment and Loading

A key principle of ComfyUI custom node development is **self-containment**. This means that all the code and configuration necessary for a specific custom node or group of nodes to function should ideally reside within a dedicated structure inside the `ComfyUI/custom_nodes` directory. ComfyUI discovers and loads custom nodes by scanning this directory when it starts up.

The **best way** to achieve self-containment and ensure ComfyUI finds your nodes is to:

*   **Organize your nodes into subdirectories** within `ComfyUI/custom_nodes`. Think of each subdirectory as a container or package for related nodes.
*   Include an **`__init__.py`** file inside each subdirectory intended as a package. This file can be empty, but its presence tells Python (and thus ComfyUI) that the directory is a package to be loaded. This is crucial for ComfyUI to recognize the directory as a custom node package.
*   Place your Python files (`.py` extension) containing the node class definitions **within these package subdirectories** or directly in `custom_nodes` for simple cases.
*   Ensure that the **required registration dictionaries**, `NODE_CLASS_MAPPINGS` and `NODE_DISPLAY_NAME_MAPPINGS`, are present at the **top level** (no indentation) in the Python file(s) that define your nodes within the `custom_nodes` structure. These dictionaries are the "header" that ComfyUI reads to identify and load your custom nodes. If these mappings are missing or incorrect, the node import will fail.

By following this structure, each custom node or package of nodes exists as an independent unit within the `custom_nodes` folder. ComfyUI loads files with the correct header from this directory, and package directories containing `__init__.py` are recognized, allowing ComfyUI to integrate your node(s) into the UI and workflow system. Restarting ComfyUI is necessary for it to scan the `custom_nodes` directory and load any new or modified nodes.

## 2. File Structure and Location

*   Custom nodes must reside within the **`ComfyUI/custom_nodes`** directory.
*   Nodes can be organized into subdirectories within `custom_nodes`.
*   Python files for nodes must have the **`.py` extension**.
*   A subdirectory intended as a package should contain an **`__init__.py`** file. This file can be empty or contain package-level code. It is essential for ComfyUI to recognize the directory as a custom node package.
*   Filenames and directory names often use **underscores (`_`)** instead of hyphens (`-`) at lower levels. Python doesn't like hyphens very much, so using underscores is safer. Avoiding both altogether is also an option.

## 3. Core Node Definition: The Python Class

Each custom node is typically defined as a **Python `class`**. It is good practice to name the class the same as the main node file.

```python
class MyCustomNode:
    # Class attributes and methods go here
    pass # Placeholder for basic structure
3.1. Essential Class Members
